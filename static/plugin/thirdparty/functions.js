...
function simpleMap(_latitude, _longitude, draggableMarker, scrollwheel, externalCall){
    ...
    if( externalCall == true ){
        ...
        var markerIcon = path + "static/img/marker.png";
        loadScript( path + "static/plugin/thirdparty/richmarker-compiled.js",renderMap);
    }
    else {
...
function createHomepageGoogleMap(_latitude,_longitude,json){
    $.get("static/plugin/thirdparty/custom.infobox.js", function() {
        gMap();
    });
    function gMap(){
        ...
        for (var i = 0; i < json.data.length; i++) {
            ...
            var infoboxOptions = {
                ...
                closeBoxURL: "static/img/close-infobox.png",
...
function jQueryLoaded(){
    ...
    function imagesLoaded(){
        loadScript( path + "static/plugin/thirdparty/imagesloaded.pkgd.min.js", jqueryColor);
        loadScript( path + "static/plugin/bootstrap/js/bootstrap.min.js", false);
    }

    function jqueryColor(){
        if( $('.average-color').length ){
            loadScript( path + "plugin/thirdparty/jquery.color-2.1.2.min.js", jqueryAverageColor);
        }
        else {
            loadFunctions();
        }
    ...
    function jqueryAverageColor(){
        loadScript( path + "plugin/thirdparty/jquery.average-color.js", loadFunctions);
    }

    function loadFunctions(){
        loadScript( path + "plugin/thirdparty/functions.js", runScripts);
    }
    ...
    function loadMap() {
        var script = document.createElement('script');
        script.type = 'text/javascript';
        script.src = 'https://maps.googleapis.com/maps/api/js?key=AIzaSyA_LszqoSTICMxYxVON_VS8f66Mo1zVHtA&sensor=false&libraries=places&callback=initialize';
